...
**Где стоит точка перехода.** Правило одно: там, где игрок физически
оказывается, придя этой дорогой. Лестница — сверху, у верхней ступени,
а не под маршем: `findNearestInteractable` меряет расстояние до центра
рамки, и рамка внутри габарита лестницы недостижима, потому что стоять
в этом месте нельзя. Дверь — перед дверью, а не в другом конце комнаты.
Это ломалось трижды (`uniWest2`/`uniEast2` toFloor1, `uniBasement` up,
`uniArchive` back), и каждый раз выглядело как «перехода нет».
...
tests/stealth.js        кража: расстановка, обход без стен, конус и стены,
