...
def readpng(p):
    ...
    while i<len(d):
        ...
        i+=12+ln
    raw=zlib.decompress(idat); out=bytearray(w*h); prev=bytearray(w)
    pos=0
    for y in range(h):
...
src,dst,x0,y0,cw,chh,scale = sys.argv[1],sys.argv[2],*map(int,sys.argv[3:])
w,h,g=readpng(src)
x0=max(0,min(x0,w-1)); y0=max(0,min(y0,h-1))
cw=min(cw,w-x0); chh=min(chh,h-y0)
W,H=cw*scale,chh*scale
out=bytearray(W*H)
for y in range(H):
    sy=y0+y//scale
    for x in range(W): out[y*W+x]=g[sy*w+x0+x//scale]
